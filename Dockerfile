...
# instead of Alpine to avoid DNS://release issues on some dependencies.

# https://hub.docker.com/r/hexpm/elixir/tags?name=1.18
...
COPY _build_cache/esbuild-linux-x64 _build/esbuild-linux-x64
RUN chmod +x _build/esbuild-linux-x64
...
COPY --from=builder --chown=nobody:root /app/_build/${MIX_ENV}/rel/agent_orch_landing ./

USER nobody

# If using an environment that doesn't automatically reap zombie processes, it is
# advised to add an init process such as tini via `apt-get install`
# above and adding an pointentry. See https://github.com/krallin/tini for details
# ENTRYPOINT ["tini", "--"]
