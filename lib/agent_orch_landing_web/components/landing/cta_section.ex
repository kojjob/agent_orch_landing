  ...
  def cta_section(assigns) do
    ...
    <section class="px-6 py-24 lg:py-32 lg:px-8">
      <div class="mx-auto max-w-3xl text-center">
        <h2
          class="font-[Space_Grotesk] text-3xl font-bold tracking-tight sm:text-4xl bg-gradient-to-r from-indigo-600 to-purple-600 dark:from-indigo-400 dark:to-purple-400 bg-clip-text text-transparent"
          data-animate="fade-up"
        >
          Ready to take control of your agents?
        </h2>
        <p
          class="mt-6 text-lg text-gray-500 dark:text-gray-400"
          data-animate="fade-up"
          data-delay="100"
        >
        ...
        <div class="mt-8">
          ...
          <% else %>
            <.form
              for={@form}
              phx-submit="submit_cta_email"
              class="flex flex-col sm:flex-row sm:items-start gap-3 justify-center max-w-md mx-auto [&_.fieldset]:mb-0"
            >
              <div class="flex-1">
                <.input
                  field={@form[:email]}
              ...
              <button
                type="submit"
                class="rounded-full bg-gradient-to-r from-indigo-600 to-purple-600 px-7 py-3 text-sm font-semibold text-white shadow-lg shadow-indigo-500/25 hover:shadow-indigo-500/40 hover:brightness-110 active:scale-[0.97] transition-all whitespace-nowrap"
              >
                Join Waitlist →
              </button>
            </.form>
      ...
      <%= if @show_partner_modal do %>
        <div
          ...
          phx-click="close_partner_modal"
        >
          <div
            class="w-full max-w-lg mx-4 rounded-2xl bg-white/80 dark:bg-gray-900 p-8 ring-1 ring-gray-200 dark:ring-gray-700 shadow-2xl"
            phx-click={%JS{}}
          >
            ...
            <%= if @partner_submitted do %>
              <div class="text-center py-8">
                ...
                <button
                  phx-click="close_partner_modal"
                  class="mt-6 rounded-lg bg-indigo-600 px-6 py-2.5 text-sm font-semibold text-white hover:bg-indigo-500 transition-colors"
                >
                  Close
                </button>
              </div>
            <% else %>
              ...
              <.form
                for={@partner_form}
                phx-submit="submit_partner"
                phx-change="validate_partner"
                class="space-y-4"
              >
                ...
                />
                <.input
                  field={@partner_form[:pain_point]}
                  type="textarea"
                  label="Biggest pain point with agents?"
                  required
                />

                <button
                  type="submit"
                  class="w-full rounded-lg bg-gradient-to-r from-indigo-600 to-purple-600 px-6 py-3 text-sm font-semibold text-white shadow-lg shadow-indigo-500/25 hover:shadow-indigo-500/40 hover:brightness-110 active:scale-[0.97] transition-all"
                >
